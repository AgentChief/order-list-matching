#!/usr/bin/env python3
"""
reconcile_and_update.py

- Merges canonical customer YAML with default configs.
- Checks for duplicate ORDER_KEYs in the order list.
- Reconciles order and shipment keys (dynamic, mapping-driven).
- Handles only selected customers for testing if desired.
- Full robust handling of missing data, NaN, and reporting.

USAGE:
  python reconcile_and_update.py \
    --canon-yaml canonical_customers.yaml \
    --orders-csv "greyson and jo.csv" \
    --ships-csv  "greyson and jo shipments.csv"
"""
...
def sanitise(colname):
    """Strip anything after '#' (YAML comments) and whitespace."""
...
def make_key(df, cols):
    ...
    # Always use fillna("") and cast to string to avoid float nan
...
def build_dynamic_key_order(df, col_map):
    """
    Builds a SHIP_KEY for orders using YAML's column_mapping,
    but only includes columns present in df.
    """
    # Only use mapping keys (order columns) that are present in the file
    used_keys = [k for k in col_map.keys() if k in df.columns]
    if not used_keys:
        # Fallback: just use whatever is in the file!
        return pd.Series([""], index=df.index)
    key_fields = []
    for k in used_keys:
        col = k
        field = df[col].fillna("").astype(str).str.upper().str.strip()
        key_fields.append(field)
    return pd.Series(["|".join(row) for row in zip(*key_fields)], index=df.index)

def build_dynamic_key_ship(df, ship_key_fields):
    """
    Builds a SHIP_KEY for shipments using unique_keys (from YAML), 
    only if present in df.
    """
    ...
    key_fields = []
    for c in used_keys:
        field = df[c].fillna("").astype(str).str.upper().str.strip()
        key_fields.append(field)
    return pd.Series(["|".join(row) for row in zip(*key_fields)], index=df.index)


def build_dynamic_key(df, key_cols, mapping=None):
    """
    Builds a key from the dataframe using key_cols.
    If mapping is given, maps order keys to shipment columns or vice versa.
    Handles missing columns with blank, all outputs are string.
    """
    key_fields = []
    for col in key_cols:
        # If mapping, use mapping[col] as target; else just use col
        src_col = mapping[col] if mapping and col in mapping else col
        # Allow for e.g. COLOUR/COLOR, look for a matching column (case-insensitive)
        found_col = None
        for c in df.columns:
            if c.strip().upper() == src_col.strip().upper():
                found_col = c
                break
        if found_col:
            key_fields.append(df[found_col].fillna("").astype(str))
        else:
            # Fill with blanks if not present
            key_fields.append(pd.Series("", index=df.index))
    return pd.Series(["|".join(row) for row in zip(*key_fields)], index=df.index)
...
def print_reconciliation_summary(
    ...
    # All keys forced to string for set ops and print
...
def reconcile_quantities(order_file, ship_file, canon, customers_to_check=None):
    ...
    # Optional: Only process selected customers for test
    ...
    # --- DEBUG: Show shipment columns in use ---
    ...
    # Build KEY for deduplication check (standard approach)
    ...
    # --- Build robust SHIP_KEY for orders and shipments ---
    # For ORDERS, use only mapping keys that exist in the DataFrame
    def build_dynamic_key_order(df, col_map):
        used_keys = [k for k in col_map.keys() if k in df.columns]
        if not used_keys:
            return pd.Series([""], index=df.index)
    ...
    # For SHIPMENTS, use only mapping values that exist in the DataFrame
    def build_dynamic_key_ship(df, ship_key_fields):
        used_keys = [c for c in ship_key_fields if c in df.columns]
        if not used_keys:
            return pd.Series([""], index=df.index)
    ...
    # Orders SHIP_KEY columns = only those mapping keys present
    orders["SHIP_KEY"] = build_dynamic_key_order(orders, col_map)
    # Shipments SHIP_KEY columns = only those mapping targets present
...
def main():
    ...
    ap.add_argument("--canon-yaml", required=True, help="Input canonical customers YAML")
    ap.add_argument("--orders-csv", required=True, help="Order lines CSV")
    ap.add_argument("--ships-csv", required=True, help="Shipments CSV")
    ap.add_argument("--out-yaml", default="customers_merged.yaml", help="Output updated YAML")
    ap.add_argument("--customers", nargs="*", help="Only process these customers (by canonical name)")
    args = ap.parse_args()
    ...
    print(f"✅ Updated YAML written to {args.out_yaml}")
...
# This file is now a utility module. All orchestration is handled in run_pipeline.py.
